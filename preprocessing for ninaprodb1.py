...
#%% multi file
dir_path = r'C:\Users\owner\projects\emg\datas\db2'
file_pathlist = []
dir_list = []
file_type = []
for (root, directories, files) in os.walk(dir_path):
...
#%% stimulus별로 data를 쪼갬.
path= r'C:\Users\owner\projects\emg\datas\db2\DB2_s1\S1_E2_A1'
res = loadmat(path)
emg = res['emg']
train = pd.DataFrame(columns=['emg','stimulus','repetition','subject','rerepetition','restimulus'])
test = pd.DataFrame(columns=['emg','stimulus','repetition','subject','rerepetition','restimulus'])
...
train.to_pickle('ninaprodb2train.pkl')
...
test.to_pickle('ninaprodb2test.pkl')
